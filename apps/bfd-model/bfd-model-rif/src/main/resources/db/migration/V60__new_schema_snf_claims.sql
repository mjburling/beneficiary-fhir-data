...
--
${logic.hsql-only}	create table public.bfd1537_snf_claims (
	...
	${logic.hsql-only} 	constraint public.bfd1537_snf_claims_pkey
	${logic.hsql-only} 	primary key (clm_id) );

${logic.hsql-only}	CREATE TABLE IF NOT EXISTS public.bfd1537_snf_claim_lines (
    ${logic.hsql-only}	clm_id						bigint NOT NULL,
    ...
    ${logic.hsql-only}	CONSTRAINT bfd1537_snf_claim_lines_pkey
...
${logic.hsql-only} insert into public.bfd1537_snf_claims(
...
${logic.psql-only} create table public.bfd1537_snf_claims as
select
...
${logic.psql-only} alter table public.bfd1537_snf_claims
...
${logic.hsql-only} insert into public.bfd1537_snf_claim_lines(
${logic.hsql-only}	clm_id,
...
--
${logic.psql-only} create table public.bfd1537_snf_claim_lines as
select
	${logic.psql-only} cast(clm_id as bigint),
	${logic.hsql-only} convert(clm_id, SQL_BIGINT),
...
ALTER TABLE IF EXISTS public.bfd1537_snf_claim_lines
    ADD CONSTRAINT bfd1537_carrier_snf_lines_clm_id_to_snf_claims FOREIGN KEY (clm_id)
    	REFERENCES public.bfd1537_snf_claims (clm_id);

CREATE INDEX IF NOT EXISTS bfd1537_snf_claims_bene_id_idx
	ON public.bfd1537_snf_claims (bene_id);

ALTER TABLE IF EXISTS public.bfd1537_snf_claims
    ADD CONSTRAINT bfd1537_snf_claims_bene_id_to_beneficiaries FOREIGN KEY (bene_id)
...
${logic.psql-only} CREATE OR REPLACE VIEW
	public.snf_claims_v as
    	select * from public.bfd1537_snf_claims;

${logic.hsql-only} CREATE VIEW
${logic.psql-only} CREATE OR REPLACE VIEW
	public.snf_claim_lines_v as
    	select * from public.bfd1537_snf_claim_lines;