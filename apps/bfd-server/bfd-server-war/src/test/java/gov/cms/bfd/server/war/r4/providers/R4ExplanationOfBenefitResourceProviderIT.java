...
public final class R4ExplanationOfBenefitResourceProviderIT {
  private static final Logger LOGGER =
      LoggerFactory.getLogger(R4ExplanationOfBenefitResourceProviderIT.class);
  ...
  public void readEobForExistingPartDEvent() throws FHIRException {
    ...
    ExplanationOfBenefit eob =
        fhirClient
            .read()
            .resource(ExplanationOfBenefit.class)
            .withId(TransformerUtilsV2.buildEobId(ClaimTypeV2.PDE, claim.getClaimId()))
            .execute();
   ...
   */
  // TODO - reconcile exception diffs
  // here is what gets thrown:
  // Caused by: ca.uhn.fhir.rest.server.exceptions.ResourceNotFoundException: HTTP 404 Not Found:
  //    Resource ExplanationOfBenefit/pde--1234 is not known
  ...
  private void clearCarrierClaimLastUpdated(long claimId) {
    LOGGER.info("clearCarrierClaimLastUpdated, claimId: " + claimId);
    ServerTestUtils.get()
        .doTransaction(
            (em) -> {
